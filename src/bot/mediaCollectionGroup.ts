...
export class MediaCollectionGroup {
  ...
  private readonly _documents: MediaCollection<
    InputMediaDocument & Pr0grammItemId
  >;
  private readonly _photosAndVideos: MediaCollection<
    (InputMediaPhoto & Pr0grammItemId) | (InputMediaVideo & Pr0grammItemId)
  >;
  ...
  private addPhotoOrVideo(
    photoOrVideo:
      | (InputMediaPhoto & Pr0grammItemId)
      | (InputMediaVideo & Pr0grammItemId)
  ) {
  ...
  async send(
    bot: Bot,
    chatId: number
  ): Promise<SendMediaCollectionGroupResult> {
    return {
  ...
  public static fromItems(
    items: Omit<Pr0grammItem, "gift">[],
    showText: boolean
  ): MediaCollectionGroup {
    const newMediaCollectionGroup = new MediaCollectionGroup();

    for (const item of items) {
      const postMarkdown = `<a href="${
        SystemService.getInstance().PR0GRAMM_SITE
      }/new/${item.id}">Post</a>`;
      const userMarkdown = `<a href="${
        SystemService.getInstance().PR0GRAMM_SITE
      }/user/${item.user}">${item.user}</a>`;
      const caption = `${postMarkdown} von ${userMarkdown}`;

      if (item.image.endsWith(".mp4")) {
        const imageUrl = `${SystemService.getInstance().PR0GRAMM_VID_CDN}/${
          item.image
        }`;
        ...
        const imageUrl = `${SystemService.getInstance().PR0GRAMM_IMAGE_CDN}/${
          item.image
        }`;
...
export type SendMediaCollectionGroupResult = {
  ...
  photosAndVideos: SendMediaCollectionResult<
    (InputMediaPhoto & Pr0grammItemId) | (InputMediaVideo & Pr0grammItemId)
  >;
};
