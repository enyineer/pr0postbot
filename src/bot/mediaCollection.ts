...
export class MediaCollection<T extends MediaType & Pr0grammItemId> {
  ...
  async sendAll(
    bot: Bot,
    chatId: number
  ): Promise<SendMediaCollectionResult<T>> {
    ...
    for (const chunk of this.asChunks(4)) {
      ...
      if (chunk.length === 1) {
        ...
        try {
          ...
          logger.warn(
            `Could not send media group, trying to send as single items`,
            chunk,
            err
          );
    ...
    switch (item.type) {
      case "audio":
        try {
          ...
          logger.error(`Could not send item`, item, err);
          return false;
        }
      case "document":
        try {
