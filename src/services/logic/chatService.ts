...
import {
  Pr0grammItemId,
  SendMediaCollectionGroupResult,
} from "../../bot/mediaCollectionGroup";
import {
  MediaType,
  SendMediaCollectionResult,
} from "../../bot/mediaCollection";
...
export class ChatService {
  ...
  private processChats = async () => {
    ...
    const itemsCollection = new Pr0grammItemCollection(
      itemsLast24HoursAfterStart
    );
    ...
    for (const chat of chats) {
      ...
      const filteredMediaCollectionGroup =
        filteredItemsCollection.toMediaCollectionGroup(chat.showText);

      const sendMediaResult = await filteredMediaCollectionGroup.send(
        this.bot,
        parseInt(chat.id.toString())
      );

      await this.saveShownMessages(
        parseInt(chat.id.toString()),
        sendMediaResult
      );

      await this.telegramChatService.updateLastUpdate(
        parseInt(chat.id.toString())
      );
  ...
  private async saveShownMessages(
    chatId: number,
    mediaCollectionGroupResult: SendMediaCollectionGroupResult
  ) {
    await this.saveSpecificShownMedia(
      chatId,
      mediaCollectionGroupResult.audios
    );
    await this.saveSpecificShownMedia(
      chatId,
      mediaCollectionGroupResult.documents
    );
    await this.saveSpecificShownMedia(
      chatId,
      mediaCollectionGroupResult.photosAndVideos
    );
  }

  private async saveSpecificShownMedia<T extends MediaType & Pr0grammItemId>(
    chatId: number,
    media: SendMediaCollectionResult<T>
  ) {
